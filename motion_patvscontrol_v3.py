	...
	with open(inFile) as motionFile:
		for line in motionFile:
			...
			if len(lineIndex) == iind:
				break
			linei+=1
	return (tra, volumes*acq_time, absol, all_tra)
...
def getFD(inFile, xColumn,lineIndex,acq_time,ref=-1, inFile2=None, lineIndex2=None):
	rot = getRotationMillimeters(inFile, xColumn, lineIndex, acq_time, ref,inFile2, lineIndex2)
	trans = getTranslation(inFile, xColumn, lineIndex, acq_time, ref,inFile2, lineIndex2)
	return (rot+trans)/2
def getFramewiseThreashold(inFile, xColumn,lineIndex,acq_time,ref=-1, inFile2=None, lineIndex2=None):
	rot = getRotationMillimeters(inFile, xColumn, lineIndex, acq_time, ref,inFile2, lineIndex2,avg=False)
	trans = getTranslation(inFile, xColumn, lineIndex, acq_time, ref,inFile2, lineIndex2, avg=False)
	fd = rot[3]+trans[3]
	c=0
	for i in fd:
		if math.sqrt(sum(i)) >.7:
			c = 1 +c
...
def plotWithinScanMotion():
	output=csv.writer(open('/space/erebus/1/users/data/scores/motion_within_scan_FD_output_140.csv','w+'))
	...
	for m in metric:
			...
			fg.subplots_adjust(left=.03, bottom=.06, right=.97, top=.95, wspace=.18, hspace=.28)
			num=0
			for name  in scans:
...
def plotBetweenScanMotion():
	...
	output=csv.writer(open('/space/erebus/1/users/data/scores/motion_between_scan_FS_output_140.csv','w+'))
	output.writerow(['subject','rot_trans','scan_type','score'])
	...
	for m in metric[0:3]:
		...
		plt.show()
		#fg.savefig("/autofs/space/erebus_001/users/data/scores/new2/plots/motion_between_scan_patvscontrol",dpi=199)
...
def plotLTAMovement():
	...
	plt.xticks(rotation=90)
	# plt.show()
	fg.savefig("/autofs/space/erebus_001/users/data/scores/new2/plots/motion_LTA_movement_patvscontrol",dpi=199)
